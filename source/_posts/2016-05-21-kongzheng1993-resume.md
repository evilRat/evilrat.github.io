...
中国移动crm项目、客服项目、中移在线北京分公司统一接口平台、一体机等项目开发。
...
### 项目介绍
中移在线一体机项目是供中国移动营业厅一体机和pad等移动设备使用的web应用，使用springboot+freemarker。调用中移在线统一接口平台完成客户业务。
### 责任描述
从零搭建并开发，因为是给一体机和pad等终端提供页面，调用统一接口平台，所以使用了较为简单快速的springboot+freemarker，并在项目中负责开发了人证比对、实名制资料登记、补卡等基础流程，组内其他成员会在此基础上根据客户需求增加功能。
...
### 项目介绍
此项目为中国移动在线公司北京分公司统一接口平台。此系统为中间层，中移在线北京分公司等10086IVR、营业厅一体机、app、网站等都通过调用此接口平台，由接口平台转接到能力提供方，再将能力响应信息返回给调用方。大部分接口要做一些数据格式的转换和数据内容的映射，以适应各个渠道的调用。此系统为分布式架构（nginx、tomcat、redis、mysql），使用ssm框架，涵盖jersey、dubbo、zookeeper等技术。通过一个web项目提供接口的配置功能，在页面可以进行接入接口、转接接口、接口映射、接入参数、转接参数、参数映射、接入渠道、渠道权限和能力等的配置。当收到调用，程序会通过调用url获取到通过redis/mysql获取到此接口的相关配置，并根据获取到的参数配置进行参数校验、参数处理、参数映射，调用转接接口获取响应后处理并返回给调用方。
### 工作内容
...
通过此需求，为系统增加了作业调度功能，为以后相似需求提供了模版，受到了领导和客户的好评。
