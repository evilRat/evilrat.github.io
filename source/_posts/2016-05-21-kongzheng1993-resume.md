...
项目为分布式架构，Spring Cloud、Nginx、Mysql、fastDfs、RabbitMQ、自研工作流eChain。使用百度AI开放平台语音识别、自然语言处理能力，智能化贷后业务。银行客户经理可使用手机app，录制贷后检查访谈内容，通过百度AI能力，进行语音识别、文本过滤降重、文本拆分成段，整合为贷后检查报告。语音文件保存到文件服务器，随时可查。支持离线语音文件保存、联网后上传处理。可根据业务进行检查模版配置，实现定制化贷后检查。主要负责百度AI接口的封装，语音文件上传、查询，语音文件格式转换等工作，并参与业务梳理与项目设计。封装了百度语音识别标准版接口（Rest）和实时语音识别接口（WebSocket）。使用ffmpeg进行语音文件格式转换，以适应百度语音识别接口。使用fastDFS进行文件存储，提供文件上传和下载接口，并支持直接从fastDFS读取输入流用于百度WebSocket接口，提高语音识别效率。
